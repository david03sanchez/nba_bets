...
def distanceSlice((game_id, team):
    ...
    homeAwayCheck = True if '@' in df1[df1['TEAM_ABBREVIATION'] == hometeam].iloc[:2, :]['MATCHUP'][1] else False
    awayAwayCheck = True if '@' in df1[df1['TEAM_ABBREVIATION'] == awayteam].iloc[:2, :]['MATCHUP'][1] else False
...
def getTeamStats(abv, latestdate):
    ...
    team_subset.iloc[:, 1:18] = team_subset.iloc[:, 0:18].ewm(halflife=7).mean()
